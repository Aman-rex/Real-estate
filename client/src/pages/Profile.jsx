...
export default function Profile() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      disptach(updateUserSucess(data));
  ...
  const handeleDeleteUser = async() => {
    try {
      ...
      disptach(deleteUserSucess(data))
    } catch (error) {
  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <form className="flex flex-col gap-4" onSubmit={handleSubmit}>
        ...
        <button
          ...
          className="bg-slate-700 text-white rounded-lg
                          p-3 uppercase hover:opacity-90 disabled:opacity-70"
        >
      ...
      <p className="text-red-700 mt-5">{showListingsError?'Error shopwing listings':''}</p>
      {
        userListing && userListing.length >0 &&
        <div className="flex flex-col gap-4">
          <h1 className="text-center my-7 text-2xl font-semibold">Your Listings</h1>
        {userListing.map((listing,index)=>(
          <div key={index} className="flex gap-3 justify-between items-center border rounded-lg p-3">
